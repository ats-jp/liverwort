...
public class EditorRootElement implements Element {
	...
	public EditorRootElement(CommandColumnRepository repository) {
		...
		Arrays.asList(repository.getIDs()).forEach(this::buildTree);
	}
	...
	private JavaElement getJavaElement(String path) {
		...
		if (element == null) {
			try {
				IJavaProject project = LiverwortPlugin.getDefault()
					.getProject();
	...
	private void buildTree(String id) {
		...
		Arrays.asList(names).forEach(name -> buildAnchor(names, name, id));
	}

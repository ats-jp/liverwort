...
public class ResourceLocator implements Comparable<ResourceLocator> {
	 ...
	 * <br>
	 * スキーマ名を省略しているので、一意に決定できるように Liverwort に設定されるスキーマは一つでなければなりません。
	 *
	 * @param tableName テーブル名
	...
	public final String getSchemaName() {
		...
		//Liverwort 初期化以前にインスタンス化されても大丈夫なように、デフォルトスキーマ名は、この時点で取得する
		String[] schemaNames = LiContext.get(LiManager.class).getConfigure().getSchemaNames();
	 ...
	 * <br>
	 * Liverwort にスキーマが一つしか設定されていない場合、テーブル名のみとなります。

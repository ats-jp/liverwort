...
import jp.ats.liverwort.sql.DeleteDMLBuilder;
...
public class DataAccessHelper {
	...
	public int count(ResourceLocator locator, Condition condition) {
		...
		LiConnection connection = manager.getConnection();
		try (LiStatement statement = connection.getStatement(builder.toString(), builder)) {
	...
	private static class PreparedStatementFacade extends StatementFacade {
		...
		private LiStatement statement;

		@Override
		void process(String sql, PreparedStatementComplementer complementer) {
			statement = LiContext.get(LiManager.class).getConnection().getStatement(sql, complementer);
		}
